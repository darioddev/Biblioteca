...
initializeSession('columnaPrestamos', 'ordenacionPrestamos', 'rowPrestamos', 'pagePrestamos', 'nombre', 'ASC', $initalize, 1);
...
if (isset($_GET["search"]) && !empty($_GET["search"]) && isset($_GET['type'])) {
    ...
    header("Location: ?ruta=prestamos&row=" . $_SESSION['rowPrestamos'] . "&column=" . $_SESSION['columnaPrestamos'] . "&order=" . $_SESSION['ordenacionPrestamos'] . "&page=" . $_SESSION["pageLibros"]);
    die();
}
...
<section class="home">
    ...
    if (isset($usuarios) && !empty($usuarios)) {
        ...
        <div class="table-options">
            <div class="table-options-row">
                <span>
                    <input type="number" name="rows" id="rows" value="<?php echo $_SESSION['rowPrestamos'] ?>" min="0" max="20">
                </span>
            </div>
    ...
    if (isset($usuarios) && empty($usuarios)) {
        ...
        $icons = array(
            ...
            array("option-link alt", "?ruta=prestamos&remove=", "fas fa-trash-alt", "borrado","prestamos","ID_Autor","verificaEstado"),
