...
if (isset($_GET["remove"]) && !empty(trim($_GET["remove"]))) {
    ...
    sql_update_estado('Libros', $_GET["remove"]);
...
if (isset($_GET["reservar"]) && !empty(trim($_GET["reservar"])) && $_SESSION['rol'] == "LECTOR") {
    sql_insertar_dato(
        'Prestamos',
        array(
    ...
    sql_update_estado('Libros', $_GET['reservar']);
...
if (isset($_GET["search"]) && !empty($_GET["search"]) && isset($_GET['type'])) {
    ...
    if (isset($_SESSION['rol']) && $_SESSION['rol'] == "LECTOR") {
        $usuarios = sql_get_all_libros(null, $initalize, null, null, $_GET["column"], $_GET["order"], true);
        $pages = pages($_GET, $initalize, count($usuarios));
        $usuarios = sql_get_all_libros($pages[0], $initalize, null, null, $_GET["column"], $_GET["order"], true);
    }
...
<section class="home">
    ...
    if (isset($usuarios) && empty($usuarios)) {
        ...
        $icons = array(
            ...
            array("option-link cog", "?ruta=libros&reservar=", "fas fa-cloud-upload-alt", "reservarLibro", $_SESSION['id']),
        );
        if (isset($_SESSION['rol']) && $_SESSION['rol'] == "LECTOR") {
